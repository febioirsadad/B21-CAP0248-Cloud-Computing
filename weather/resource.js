...
function formatDataBmkg(json) {
    const data = json.data.forecast[0].area
        ...
        .map((area) => {
            ...
            // get the minimum humidity value
            const min_hum = area.parameter
                .filter((params) => {
            ...
            const max_hum = area.parameter
                .filter((params) => {
                    return params.$.id === "humax";
                })
                .map((params) => {
                    return params.timerange.map((timerange) => {
                        const dateTime = timerange.$.day;
            ...
            const wind = area.parameter
                ...
                .map((params) => {
                    return params.timerange.map((timerange) => {
                        ...
                        return {
                            date: dateTime,
                            value: timerange.value[2]._
                        }
                    })
                });

            // get the wind direction value
            const wdirection = area.parameter
                .filter((params) => {
                    return params.$.id === "wd";
                })
                .map((params) => {
                ...
                parameter: [
                    {
                        ...
                        min_hum: min_hum[0][0].value,
                        max_hum: max_hum[0][0].value,
                        wind: wind[0][0].value,
                        wdirection: wdirection[0][0].value,
                    ...
                    },
                    {
                        ...
                        min_hum: min_hum[0][1].value,
                        max_hum: max_hum[0][1].value,
                        wind: wind[0][1].value,
                        wdirection: wdirection[0][1].value,
                    ...
                    },
                    {
                        ...
                        min_hum: min_hum[0][2].value,
                        max_hum: max_hum[0][2].value,
                        wind: wind[0][2].value,
                        wdirection: wdirection[0][2].value,
